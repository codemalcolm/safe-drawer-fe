...
export default function AddCardModal({
  ...
  const [isAuthorized, setIsAuthorized] = useState(defaultValues?.isAuthorized ?? true);
  ...
  function validate() {
    ...
    if (!trimmed) { e.cardId = "ID karty je povinné."; }
    else if (!isEdit && cards.some((c) => c.cardId === trimmed && c.drawerId === drawerId)) {
  ...
  function handleSubmit() {
    ...
    if (isEdit && cardRecordId) {
      ...
      addSystemLog({ timestamp: new Date().toISOString(), type: "info", message: `Karta přidána – ${trimmedId}`, drawerId });
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <input
              type="text"
              value={cardId}
              onChange={(e) => { setCardId(e.target.value); setErrors((p) => ({ ...p, cardId: undefined })); }}
            ...
            {isEdit && <p className="text-slate-400 text-xs mt-1">ID karty nelze změnit.</p>}
          </div>
          ...
          <div className="flex items-center justify-between py-1">
            <span className="text-sm font-medium text-slate-700">Autorizace</span>
            <button
              type="button"
          ...
          <p className="text-xs text-slate-400 -mt-2">{isAuthorized ? "Přístup povolen" : "Přístup zakázán"}</p>
        </div>

        <div className="flex gap-3 px-6 py-4 border-t border-slate-100">
          <button onClick={onClose} className="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-700 hover:bg-slate-50 transition-colors font-medium">
            Zrušit
          </button>
          <button onClick={handleSubmit} className="flex-1 px-4 py-2.5 bg-sky-600 text-white rounded-xl text-sm font-semibold hover:bg-sky-700 transition-colors">
