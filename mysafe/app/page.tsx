        ...
        <div className="flex flex-col gap-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            ...
            <dl className="space-y-3">
              ...
              <DeviceRow
                label="Čekající události"
                value={String(device.pendingEvents)}
                highlight={device.pendingEvents > 0}
              />
              <DeviceRow
                label="Incidenty celkem"
              ...
              <ul className="space-y-2">
                {incidents.map((i) => (
                  <li key={i.id} className="text-sm text-amber-800">
                    ...
                    <br />
                    {i.incidentType === "forcedOpening"
                      ? "Pokus o násilné otevření"
    ...
    <div className="flex justify-between items-baseline py-2 border-b border-slate-100 last:border-none">
      ...
      <dd
        className={`font-mono text-xs ${
          highlight ? "text-amber-600 font-bold" : "text-slate-700"
        }`}
      >
        {value}
      </dd>
